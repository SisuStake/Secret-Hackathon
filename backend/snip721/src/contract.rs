...
pub fn execute(deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg) -> StdResult<Response> {
    ...
    let response = match msg {
        ExecuteMsg::DepositSilk {} => {
            ...
            // Send the SILK tokens to the contract address
            send_tokens(&info.sender, &contract_address, &amount)?;

            // Increase the balance of the contract in the state
            let mut contract = SecretContract::from_address(&contract_address)?;
            contract.inc_balance(amount)?;

            // Return a success response with an event
            let mut attributes = vec![attr("action", "deposit_silk"), attr("amount", amount.to_string())];
            attributes.push(attr("contract_address", contract_address));
            Response::new().add_attributes(attributes)
        },
        ExecuteMsg::MintNft {
            token_id,
            owner,
